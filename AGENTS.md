...
- Attach Jira only to the ticket coordinator. It reads and posts an outcome comment for every completed investigation, including successful findings, using REST API v2; never fall back to v3. Preserve same-execution retry deduplication and never blindly retry an uncertain POST.
