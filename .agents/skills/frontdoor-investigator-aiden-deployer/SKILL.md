...
- Verify that the legacy request-information stage posts an outcome even for diagnosed or closed tickets and preserves the existing stage/resource names. The final output retains clarification_result but uses delivery statuses posted/already_posted/delivery_unknown/integration_error; ready_to_reinvestigate is now a boolean. Report this contract change to callers.
- Run the offline mocked plan tests with `terraform test` (Terraform 1.7+). OpenTofu 1.11.5 with StackGen 0.1.41 fails initialization for this mock suite, although `tofu validate` works. For an authorized runtime test, follow tests/clarification-cases.md at the repository root. Check API v2 requests, successful outcome comments, blocked evidence-stage behavior, targeted questions, and same-execution retry suppression. These are agent/tool behaviors, not guarantees from Terraform validation or delivery guarantees when a workflow fails early.
- This module creates no webhook or reply-resumption rule. An external caller must invoke it again after developer replies, filter automation-authored comments, and serialize executions per ticket to reduce concurrent duplicate comments. Recommend optional execution_id input: stable across retries, unique for each distinct investigation. Without this or an exposed stable platform execution ID, cross-invocation retry deduplication is not guaranteed.
